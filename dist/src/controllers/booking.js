...
let BookingController = class BookingController {
    constructor(service) {
        this.delete = async (request, response) => {
            try {
                const data = await this.service.delete(request.query.bookingId?.toString());
        ...
        this.findOne = async (request, response) => {
            try {
                const data = await this.service.findOne(request.query.bookingId?.toString());
                response.status(http_status_1.default.OK).send(data);
            }
            catch (error) {
                response
...
BookingController = __decorate([
    ...
    __param(0, (0, inversify_1.inject)(export_1.SERVICE_IDENTIFIER.IBookingService))
], BookingController);
